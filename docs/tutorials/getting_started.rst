...
To do that, we can use the :meth:`Chart.options <highcharts_maps.chart.Chart.options>`
property. Having assembled our chart following the instructions above, ``my_chart``
already contains a :class:`HighchartsOptions <highcharts_maps.options.HighchartsOptions>`
instance in the :meth:`Chart.options <highcharts_maps.chart.Chart.options>` property.
You can access the :class:`LineSeries <highcharts_maps.options.series.area.LineSeries>`
we created at :meth:`Chart.options.series <highcharts_maps.options.HighchartsOptions.series>`,
and you can set any other options you need on 
:meth:`Chart.options <highcharts_maps.chart.Chart.options>`.

For example, let's say we want to set the chart's title to "My First Chart". To do that,
we can configure the 
:meth:`Chart.options.title <highcharts_maps.options.HighchartsOptions.title>` property
using either a :class:`Title <highcharts_maps.options.title.Title>` instance, or a
...
call the :meth:`.display() <highcharts_maps.chart.Chart.display>` method on 
``my_chart``:
...
:meth:`.download_chart() <highcharts_maps.chart.Chart.download_chart>` method:
...
:meth:`.to_js_literal() <highcharts_maps.chart.Chart.to_js_literal>` method
on ``my_chart``.
