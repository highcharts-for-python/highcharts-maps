...
These standard methods - with near-identical syntax - are available:
  ...
  * On the :class:`options.data.Data <highcharts_maps.options.data.Data>` 
    class
...
is to call :class:`Chart.from_pandas() <highcharts_maps.chart.Chart.from_pandas>` like
so:
...
column to the resulting series' data points' :meth:`.x <highcharts_maps.options.series.data.bar.BarData.x>`,
...
Now we're getting somewhere! First, we changed our series type to a :class:`ColumnSeries <highcharts_maps.options.series.bar.ColumnSeries>` to make it (a little) easier to read. Then we  added a list of column names to the ``'y'`` key in the ``property_map``  argument. Each of those columns has now produced a *separate* :class:`ColumnSeries <highcharts_maps.options.series.bar.ColumnSeries>` instance - but they're all still sharing the ``'Geographic Area'`` column as their :meth:`.x <highcharts_maps.options.series.data.bar.BarData.x>` value.
  ...
  .. note::
    ...
    :exc:`HighchartsPandasDeserializationError <highcharts_maps.errors.HighchartsPandasDeserializationError>`.
...
:meth:`ColumnSeries.point_padding <highcharts_maps.options.series.bar.ColumnSeries.point_padding>` property to a
...
column, while the data point :meth:`.x <highcharts_maps.options.series.data.bar.BarData.y>` values come from each additional column (e.g. ``'2010'``, ``'2011'``, ``'2012'``, etc.)
  ...
  .. tip::
    ...
    For more information, please see:
      ...
      * :class:`Chart.from_pandas_in_rows() <highcharts_maps.chart.Chart.from_pandas_in_rows>`
      * :class:`SeriesBase.from_pandas_in_rows() <highcharts_maps.options.series.base.SeriesBase.from_pandas_in_rows>`
...
as :class:`Chart.from_pandas() <highcharts_maps.chart.Chart.from_pandas>` *except for* ``options_kwargs`` and
...
Unlike when calling :meth:`Chart.from_pandas() <highcharts_maps.chart.Chart.from_pandas>`, we
...
57 :class:`LineSeries <highcharts_maps.options.series.area.LineSeries>` instances.
...
:meth:`.load_from_pandas() <highcharts_maps.options.series.base.SeriesBase.load_from_pandas>` method.
...
the ``series_index`` argument tells it to only use the 10th series generated.
  ...
  .. caution::
    ...
    While the :meth:`.load_from_pandas() <highcharts_maps.options.series.base.SeriesBase.load_from_pandas>`
    method supports the same arguments as 
    :meth:`.from_pandas() <highcharts_maps.options.series.base.SeriesBase.from_pandas>`, it expects that
